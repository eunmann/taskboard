...
- Use `grep -l "^status: open" .taskboard/*.yaml` to find candidate files.
- Check only those files for `blocked-by` refs (use `grep -l "blocked-by" <candidates>` to narrow further).
- Read the narrowed set of files to confirm blockers are resolved (`status: done` or `status: cancelled`).

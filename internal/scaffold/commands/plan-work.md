...
2. **Explore the codebase** — identify affected files, packages, and patterns.
3. **Decompose** — split the work into discrete tasks, each representing 30-90 minutes of focused AI work.
4. **Define dependencies** — use `blocked-by` refs so tasks can be executed in order.
5. **Create task files** — write YAML files to `.taskboard/`.
